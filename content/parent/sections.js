...
const GRADES = {
  g4: {
    ...
    teach: [
      ...
      'Ads and scams: “free Robux” is always a trap; nothing free exists (Kaspersky logged 1.6M Roblox-themed attacks in a year, so make it a game to spot them).',
  ...
  g7: {
    ...
    teach: [
      ...
      '<strong>Prebunking, formally:</strong> watch the Cambridge/Jigsaw inoculation videos together (5 manipulation techniques). Then lateral reading (Stanford Civic Online Reasoning). Booster doses matter, so make “what’s this clip trying to make you feel?” a running family game.',
      'Name the manosphere on-ramp <em>before the feeds arrive</em>: “There’s a genre that tells you someone stole something from you, sold by guys monetizing your anger, and researchers watch it hit new accounts in 23 minutes.” Boys respond better to “you’re being farmed” than “that’s bad.”',
    ],
...
const DEVICE_LAYER = {
  apple: [
    ...
    '<strong>Reliability warning:</strong> Screen Time has a long history of silently failing, so treat it as <em>policy signal, not enforcement</em>; the network layer is your backstop.',
  ],
  google: [
  ...
  amazon: [
    'Fire tablets run <strong>Fire OS</strong> (a fork of Android) with the <strong>Amazon Appstore</strong>, so <strong>Google Family Link and Google Play don’t apply</strong>. The native tool is <strong>Amazon Kids</strong>, free and built in.',
    'Create a <strong>child profile</strong> (Settings → Profiles &amp; Family Library, or the <strong>Parent Dashboard</strong> at parents.amazon.com) with a true birthdate, and the child gets the walled Amazon Kids experience, not the adult tablet.',
...
export default [
  {
    body: (h) => `
      ...
      <p><strong>For:</strong> ${h.platforms.map((p) => p.name).join(' + ')} household, ${h.kid} at age ${h.cfg.age}${h.schoolDevice ? ', a school-issued device that comes home' : ''}. Younger siblings age into each stage.
        ...
        AAP/APA/eSafety guidance (footnotes at the end).
        <strong>Kid's link:</strong> <a class="kidlink-jump" href="#share">here</a> (the safe version, further down).</p>`,
  },
...

  {
    body: (h) => `
      ...
      <div class="layer"><span class="layer-label">Layer 4: Platforms (configure as each is unlocked)</span><div class="layer-body">${list([
        '<strong>YouTube:</strong> YouTube Kids → supervised account: Explore (~9+) → Explore More (~13+) → Most of YouTube. Supervised accounts can’t comment or upload, which is half the value.',
...

  {
    body: () => `
      <aside id="share">
        ...
        <h2>Send this to your kid</h2>
        <p>A safe, kid-facing version of this plan: nothing candid, no threat model. This is the link to hand over.</p>
        <div class="share-row">
