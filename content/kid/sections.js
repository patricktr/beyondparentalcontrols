...
const GRADES = {
  ...
  g6: {
    lines: [
      ...
      ['Still waiting', () => `social media like TikTok, Instagram, and Snapchat. We’ll explain <em>why</em>, not just say no.`],
  ...
  g7: {
    lines: [
      ...
      ["You'll get good at", () => `noticing when someone online is trying to trick or pressure you, and spotting videos that are built to make you angry on purpose (some people make money that way, so don’t be their product).`],
    ],
    notes: [
  ...
  g8: {
    ...
    notes: [
      ['Why “maybe”?', `Two reasons. One: only if you’re ready. Two: the law might say not yet. Some places have set the minimum age for social media at 16, and that could be the rule where we live by the time you get here. If it is, we follow it, and the app waits until you’re old enough.`],
  ...
  // hero + journey bar + greeting + big idea
  {
    body: (h) => {
      ...
      <p class="intro"><strong>${h.hasName ? `Hey ${h.name}!` : 'Hey!'}</strong> This is the plan for how you’ll use
        the internet as you grow up. You and your parents are a team on this, so talk about it, ask
        questions, and work through it together. Here’s the big idea:</p>
